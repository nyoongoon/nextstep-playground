...
## 숫자야구게임 - 제이슨 강의
- 처음부터 바로 프로그램을 만들기 어려움 -> 먼저 어떻게 구현할지 고민 -> 요구사항 정리
- 요구사항에서 기능 목록 추출하기
### 숫자야구 기능목록 & 기능 목록에 기능 이름 붙이기
- 1부터 9까지의 서로 다른 임의의 수 3개를 생성한다 NumberGenerator#createRandomNumbers()
- 컴퓨터의 수와 플레이어의 수를 비교할 수 있다 Referee#compare() 
- - 몇 개의 숫자가 같은지 알 수 있다. - Jugment#correctCount()
- - 특정 자리에 특정 숫자가 있는지 알 수 있다 - Jugment#hasPlace() 
- 같은 수가 같은 자리 -> 스트라이크
- 같은 수가 다른 자리 -> 볼
- 같은 수가 없으면 -> 낫싱
### 객체지향 간단 설명
- static 변수 : 클래스 변수 / 일반변수 : 인스턴스 변수
- 1. 기능을 가지고 있는 클래스를 인스턴스화(객체화)한다
